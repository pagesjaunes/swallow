...
def get_and_parse(p_inqueue,p_outqueue,p_process,*args):
	"""
	...
	while True:
		try:
			...
			# Tuple => list
			listargs = list(args)
			# Adding the inputdoc to the parameter list : position #0
			listargs.insert(0,in_doc)

			# Call the proc with the arg list (keeping the * means : unwrap the list when calling the function)
			out_doc = p_process(*listargs)
